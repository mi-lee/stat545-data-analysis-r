...
ggplot(min.max, aes(x = continent, y = value, fill = continent)) + 
  geom_point(lwd=3) + # the min and max values
  geom_line(aes(colour=continent)) + # the line to connect the values
  theme(plot.title = element_text(lineheight=.8, face="bold", size = 20)) + guides(fill = F) 
...
```{r}
ggplot(gtbl, aes(x = continent, y = gdpPercap, fill = continent)) + geom_point(alpha=3/4, aes(colour=continent), position="jitter") + # jitter spreads out the points 
