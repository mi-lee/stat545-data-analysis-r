...
my.lm.fcn <- function(x, ...) {
  ...
  estCoefs <- coef(model)
  estCoefs<-rbind(estCoefs, as.data.frame(matrix(ncol=2, nrow=11, NA)))
  colnames(estCoefs) <- c("intercept", "slope")
  ...
  colnames(total.frame) <- c("intercept", "slope", "residuals", "effects", "fitted vals")
  return(total.frame)
}
